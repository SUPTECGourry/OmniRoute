...
Hermes Agent and OpenClaw users can route Grok subscription traffic through
OmniRoute by connecting the `xai-oauth` provider in Dashboard → `Providers` and
using models such as `xai-oauth/grok-build-0.1` through the normal OpenAI-
compatible endpoint. If xAI returns HTTP 403 at inference time, use the API-key
provider `xai` instead.
